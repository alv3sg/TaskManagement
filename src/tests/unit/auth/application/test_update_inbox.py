...
from src.auth.application.user_cases import UpdateInbox
...
def test_update_inbox_success(mock_inbox_repository):
    # Arrange
    ...
    mock_inbox.description = "Original description"
    ...
    # Create the use case
    update_inbox = UpdateInbox(inboxes=mock_inbox_repository)

    # Act
    result = update_inbox.execute(
        inbox_id=inbox_id, description=new_description)

    # Assert
    mock_inbox_repository.get_by_inbox_id.assert_called_once_with(inbox_id)
    mock_inbox.update.assert_called_once_with(new_description)
    mock_inbox_repository.save.assert_called_once_with(mock_inbox)
    assert result == mock_inbox


def test_update_nonexistent_inbox(mock_inbox_repository):
    # Arrange
    ...
    with pytest.raises(ValueError, match=f"Inbox not found"):
        update_inbox.execute(inbox_id=inbox_id, description="New description")
