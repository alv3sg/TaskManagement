...
from src.auth.application.user_cases import DeleteInbox
...
def test_delete_inbox_success(mock_inbox_repository):
    # Arrange
    ...
    # Create the use case
    delete_inbox = DeleteInbox(inboxes=mock_inbox_repository)

    # Act
    result = delete_inbox.execute(inbox_id=inbox_id)

    # Assert
    mock_inbox_repository.get_by_inbox_id.assert_called_once_with(inbox_id)
    mock_inbox.delete.assert_called_once()
    mock_inbox_repository.save.assert_called_once_with(mock_inbox)
    assert result == mock_inbox


def test_delete_nonexistent_inbox(mock_inbox_repository):
    # Arrange
    ...
    with pytest.raises(ValueError, match=f"Inbox not found"):
        delete_inbox.execute(inbox_id=inbox_id)
